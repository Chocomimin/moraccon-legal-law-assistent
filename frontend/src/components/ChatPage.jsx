...
export default function ChatPage() {
  ...
  // Persist chat messages on update
  useEffect(() => {
  ...
  body: JSON.stringify({ question: input, lang: "en" }),
});
      ...
      if (response.ok) {
        const answerMessages = data.answer
          .split(/\n(?=\d+\.)/) // split numbered points
          .map((line) => ({ text: line.trim(), sender: "bot" }));
      ...
      setMessages((prev) => [
        ...prev,
        { text: "Server error or unauthorized. Please login again.", sender: "bot" },
      ]);
    } finally {
      setLoading(false);
    }
  ...
  if (!user) {
    return (
      <div className="w-full min-h-screen flex items-center justify-center">
        <p className="text-gray-500">You are not logged in. Please login to access the chat.</p>
      </div>
    );
  }
    ...
    <div className="w-full min-h-screen bg-white">
      <div className="max-w-3xl mx-auto mt-10 border border-gray-200 rounded-2xl flex flex-col h-[80vh] overflow-hidden shadow-md">
        ...
        <div className="flex-1 p-6 overflow-y-auto space-y-4 scrollbar-thin scrollbar-thumb-gray-300 scrollbar-track-gray-100">
          ...
          {loading && (
            <p className="text-gray-500 text-sm italic animate-pulse">AI is typing...</p>
          )}
